...
class GaussianModel:
    ...
    def apply_mask(self, mask):
        for grad in [self._xyz.grad,
                    self._features_dc.grad,
                    self._features_rest.grad,
                    self._scaling.grad,
                    self._rotation.grad,
                    self._opacity.grad,]:
    ...
    def reset_opacity(self):
        if self.size_fixedGS and (not self.if_dynamic_opacity): # if we want to fix opacity for the previous layers
            opacities_new = torch.cat((self._opacity[:self.size_fixedGS], inverse_sigmoid(torch.min(self.get_opacity[self.size_fixedGS:], torch.ones_like(self.get_opacity[self.size_fixedGS:])*0.01))), dim=0)
        else: # if we want to update opacity for the previous layers
    ...
    def load_foundation_gs(self, path):
        plydata = PlyData.read(path)
    ...
    def prune_points(self, mask, size_fixedGS=None):
        if size_fixedGS:
            # the last size_fixedGS elements in mask should be False
            mask[:size_fixedGS] = False
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.prune_points(prune_filter, self.size_fixedGS)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        self.prune_points(prune_mask, self.size_fixedGS)
