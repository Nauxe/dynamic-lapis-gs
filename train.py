...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, foundation_gs_path=None, dynamic_opacity=False, dynamic_lapis=False, initial_gs_path=None):
    ...
    if foundation_gs_path: # settings for LapisGS
        gaussians = gs_merge(foundation_gs, gaussians) # merge gaussians
        gaussians.training_setup(opt)
        gaussians.size_fixedGS = foundation_gs._xyz.shape[0] # set the number of fixed gaussians
        if dynamic_opacity:
            gaussians.if_dynamic_opacity = dynamic_opacity # set the dynamic opacity flag
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # cv2.imwrite(f"{iteration}.png", (image.permute(1, 2, 0).cpu().detach().numpy() * 255).astype(np.uint8))
        # viewspace_point_tensor, visibility_filter, radii = viewspace_point_tensor[:gaussians.get_xyz.shape[0]], visibility_filter[:gaussians.get_xyz.shape[0]], radii[:gaussians.get_xyz.shape[0]]
...
def gs_merge(foundation_gs, train_gs):
    # 1. foundation GS
    ...
    # 3. merged GS
    merged_gaussians = GaussianModel(foundation_gs.max_sh_degree)
